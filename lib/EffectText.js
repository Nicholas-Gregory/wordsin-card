...
export default class EffectText {
    ...
    getChangedValue() {
        return this.parse.groups.changedValue;
    }
    ...
    fixSpanIncidental() {
        const isEnd = !this.getGrimoireTargeting()
        
        if (this.getTimeModifierAmount() > 1) {
            if (isEnd) {
                this.text = `${this.text}${this.text.at(-1) === 's' ? '' : 's'}`;

                return this.parseText();
            } else {
                return this.replaceText('spanIncidental', 's');
            }
        } else {
            return this.replaceText('spanIncidental', '');
        }
    }

    fixGrimoireTargetingIncidental() {
        const targeting = this.getGrimoireTargeting();

        if (targeting === 'independent') {
            return this.replaceText('grimoireTargetingIncidental', ' target ')
        } else if (targeting === 'card') {
            return this.replaceText('grimoireTargetingIncidental', `'s target `)
        } else {
            return this.replaceText('grimoireTargetingIncidental', ` `);
        }
    }
    ...
    setGrimoireTargeting(targeting) {
        if (lists.TARGETING.includes(targeting)) {
            return this
            .replaceText(`grimoireTargeting`, targeting)
            .fixGrimoireTargetingIncidental()
        } else {
            throw new TextGenerationError(`'${targeting}' is not a valid grimoire targeting keyword`);
        }
    }
