...
// Spaces in below regex patterns are important, they make sure the capture groups are not undefined, LEAVE THEM BE
...
export const effectRegExp = `((?<outerTargeting>${getListRegExp('TARGETING')})(?<outerIncidental>${targetIncidental})(?<outerTarget>${getListRegExp('TARGET')})(?<outerPluralization>${pluralization})?)?(?<keywords>((${getListRegExp('KEYWORDS')})(?<verbIncidental>${pluralization})?)+)(?<amount>${amount})? ?(?<actionIncidental>${actionIncidental})?((?<changedValue>((${getListRegExp('KEYWORDS')}) ?)+|${getListRegExp('TARGETING')}|${getListRegExp('TARGET')}|${getListRegExp('TIME')})y? for )?((?<targeting>${getListRegExp('TARGETING')})(?<targetIncidental>${targetIncidental}))?(?<target>${getListRegExp('TARGET')})(?<mainPluralization>${pluralization})?(${timeModifier})? ?(into )? ?(?<grimoireTargeting>${getListRegExp('TARGETING')})?(?<grimoireTargetingIncidental>${targetIncidental})?(grimoire)?`;