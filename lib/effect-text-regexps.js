...
export const modifierEffect = `(?<instruction>${modifierInstruction}) "(?<argument>${modifierArgument}) ?(?<words>${words})?" (to|by|from) (?<amount>${amount})?( for )?(?<targetType>${targetType})(?<targetIncidental>${targetIncidental})? effect(s)?(${timeModifier})?`;