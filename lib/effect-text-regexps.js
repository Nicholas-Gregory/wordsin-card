...
export const stateEffect = `(?<instruction>${stateInstruction}) ?((?<amount>${amount})|(?<words>${words}))? ?(?<incidental>to|for)? ?((?<stat>${stat}) for)? ?(?<targetType>${targetType})('s)?( target)? (?<targetClass>${targetClass})(${timeModifier})?`