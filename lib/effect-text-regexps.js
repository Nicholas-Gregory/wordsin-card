...
export const effectEffect = `(?<instruction>${effectInstruction})( (to|from))? (?<targetType>${targetType})( target)? (?<targetClass>${effectTargetClass}) (to )?(((?<amount>${amount}) time(?<copyIncidental>s)?)|(?<effects>${effects})|((?<effectTargetType>${targetType}) target effect))(${timeModifier})?`;