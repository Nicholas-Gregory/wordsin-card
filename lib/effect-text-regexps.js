...
export const targetClass = 'character(s)?|enem(y|ies)|all(y|ies)|object(s)?|spell(s)?|effect(s)?|offensive effect(s)?|defensive effect(s)?|utility effect(s)?|you';
...
export const modifierArgument = 'Deal|Heal|Increase|Decrease|Set|Apply|Multiply|Divide|Discard|Reveal|Draw|Put|Shuffle|Copy|Add|Subtract|Gain|Lose';
export const modifierEffect = `(?<instruction>${modifierInstruction}) ("(?<argument>${modifierArgument}) ?(?<words>${words})?"|instruction|target type|focus cost) (to|by|from) ((?<amount>${amount})|"(?<changedInstruction>(${stateInstruction})|(${modifierInstruction}))"|"((?<changedTargetType>${targetType})('s)?( target)?)"|((?<changedFocusCost>${amount}) focus))?( for )?(?<targetType>${targetType})(?<targetIncidental>${targetIncidental})? effect(s)?(${timeModifier})?`;
...
// Focus
export const focusInstruction = `gain|lose`;
export const focusTargetType = `You|All|Independent|Card`;
export const focusTargetClass = `character(s)?|enem(y|ies)|all(y|ies)`;
export const focusEffect = `(?<targetType>${focusTargetType})('s)?( target)? (?<targetClass>${focusTargetClass})? ?(?<instruction>${focusInstruction})(?<incidental>s|es)? (?<amount>${amount}) focus`;
