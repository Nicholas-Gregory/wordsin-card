...
export const modifierArgument = 'Deal|Heal|Increase|Decrease|Set|Apply|Multiply|Divide|Discard|Reveal|Draw|Put|Shuffle';
...
// Card
export const cardTargetingType = `Target|All|You`;
export const cardTargetClass = `character(s)?|enem(y|ies)|all(y|ies)`
export const cardInstruction = `discard(s)?|reveal(s)?|draw(s)?|put(s)?|shuffle(s)?`;
export const cardModifier = `chosen|target|random`;
export const topOrBottom = `top|bottom`;
export const grimoireTargeting = `target|their`;
export const cardEffect = `(?<cardTargetingType>${cardTargetingType})( (?<cardTargetClass>${cardTargetClass}))? (?<cardInstruction>${cardInstruction}) (?<amount>${amount}) ((?<cardModifier>${cardModifier}) )?card(?<cardIncidental>s)? ?((on (?<topOrBottom>${topOrBottom}))|(into))? ?(?<grimoireTargeting>${grimoireTargeting})? ?(grimoire)?`;