export default class Effect {
    constructor(text, target, time) {
        ...
        this.stateMethodTable = {
            set: this.setState,
            add: this.addState,
            subtract: this.subtractState,
            multiply: this.multiplyState,
            divide: this.divideState
        }
    }
    ...
    resolveState() {
        ...
        const property = keywords[1];

        this.stateMethodTable[keyword].call(this, property);
    }
    ...
    applyState() {
        ...
        if (keywords.length > 1) {
            for (let keyword of keywords) {
                this.target.setState(keyword, true);
            }
        } else {
            amount = this.text.getAmount();

            this.target.setState(keywords[0], amount);
        }
    }
