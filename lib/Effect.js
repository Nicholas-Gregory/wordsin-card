export default class Effect {
    ...
    isHealModifier() {
        return this.text.includes("independent target defensive effect's heal");
    }

    getHealModifier() {
        if (this.isHealModifier()) {
            const split = this.#split();
    ...
    isStatModifier() {
        return /(awareness|power|wellness|agility|swiftness) by/.test(this.text);
    }

    getStatModifierAmount() {
        if (this.isStatModifier()) {
            const split = this.#split();
    ...
    getModifiedStat() {
        if (this.isStatModifier()) {
            return this.text.match(/(awareness|power|wellness|agility|swiftness)/)[0];
        }
    }
}